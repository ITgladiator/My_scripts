...
if len(sys.argv) < 2:
    ...
    env = setting.pre_host_config["env"]  # 部署环境（test、prod等）
    time_stamp = time.strftime("%Y-%m-%d-%H-%M")  # 时间
    svn_ver_status = get_svn.get_ver(src_dir)  # 当前版本库版本号（svn info）
    log_file = "%s/pre_%s_svn_ver.log" % (setting.log_dir, siteName)
    if svn_ver_status != -1:
...
Filename : %s\033[0m''' % full_tar_filename
    ...
    hosts_list = setting.pre_host_config["hosts_list"]
    port = setting.pre_host_config["port"]
    user = setting.pre_host_config["user"]
    pkey_file = setting.pre_host_config["pkey_file"]
...
# ========================================================================================================
    #  发送配置文件
    nodeName_list = setting.pre_host_config["nodeName_list"]
