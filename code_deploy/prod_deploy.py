...
if len(sys.argv) < 2:
    ...
    siteName = sys.argv[2]
    pre_log_file = "%s/pre_%s_svn_ver.log" % (setting.log_dir, siteName)
    # 获取预发布服务器的svn版本号
    with open(pre_log_file) as pre_f:
        try:
            pre_svn_ver = int(pre_f.read())
        except Exception, e:
            print Exception, ":", e
            print "\033[31;1mError! Get pre svn version failed!!\033[0m"
            sys.exit()
    ...
    # 执行svn命令co代码
    status = get_svn.get_code(dst_dir, svn_url, svn_user, svn_pass, pre_svn_ver)
    if status == 0:
    ...
    env = setting.prod_host_config["env"]  # 部署环境（test、prod等）
    time_stamp = time.strftime("%Y-%m-%d-%H-%M")  # 时间
    svn_ver_status = get_svn.get_ver(src_dir)  # 当前版本库版本号（svn info）
    log_file = "%s/prod_%s_svn_ver.log" % (setting.log_dir, siteName)
    if svn_ver_status != -1:
...
Filename : %s\033[0m''' % full_tar_filename
    ...
    hosts_list = setting.prod_host_config["hosts_list"]
    port = setting.prod_host_config["port"]
    user = setting.prod_host_config["user"]
    pkey_file = setting.prod_host_config["pkey_file"]
...
# ========================================================================================================
    #  发送配置文件
    nodeName_list = setting.prod_host_config["nodeName_list"]
    for nodeName in nodeName_list:
        sendConfigFile.sendConfig(nodeName, siteName, env)
